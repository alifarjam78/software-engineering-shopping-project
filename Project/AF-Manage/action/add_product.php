...
if(isset($_POST["sb"]))
{
	...
	$_SESSION["price"] = $require_input["price"];
	$_SESSION["discount"] = $require_input["discount"];
	$_SESSION["amount"] = $require_input["amount"];
	...
	else
	{
		...
		else
		{
			...
			$q = $cn->pdo->prepare("INSERT INTO prodect(img,cat_id,name,name_en,about,about_en,text,text_en,price,amount,discount,autor,keyword,keywords_en,time) VALUES(:img,:category,:title,:title_en,:text,:text_en,:mor_text,:more_text_en,:price,:amount,:discount,:autor,:keyword,:keyword_en,:time)");
			$q->execute(array(
				":img" => $upload_address,
				...
				":mor_text" => $require_input["more_text"],
				":more_text_en" => $require_input["more_text_en"],
				":price" => $require_input["price"],
				":amount" => $require_input["amount"],
				":discount" => $require_input["discount"],
				":autor" => $input["autor"],
